...
def create_model(args, model_class, config=None, **kwargs):
...
def create_tokenizer(args, **kwargs):
...
class P5(T5ForConditionalGeneration):
    ...
    def __init__(self, config):
        ...
        self.n_embd = config.d_model // config.num_heads
    ...
    def forward(
        ...
        prompt_key_values = self.get_prompt(batch_size=batch_size, prefix_num=prefix_num)
...
class P5MixPrefix(T5ForConditionalGeneration):
    _keys_to_ignore_on_load_missing = [
        r"encoder\.embed_tokens\.weight",
        r"decoder\.embed_tokens\.weight",
        r"lm_head\.weight",
...
@dataclass
class T5Seq2SeqLMOutput(ModelOutput):
    """
