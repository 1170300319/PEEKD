...
class BertPrefixForMultipleChoice(BertPreTrainedModel):
    ...
    def forward(
        ...
        past_key_values = self.get_prompt(batch_size=batch_size * num_choices, prefix_num=prefix_num)
...
class BertMixPrefixForMultipleChoice(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.config = config
...
class RobertaMixPrefixForMultipleChoice(RobertaPreTrainedModel):
    _keys_to_ignore_on_load_missing = [r"position_ids"]
