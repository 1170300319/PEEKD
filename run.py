...
def evaluate(trainer):
    ...
    logits = []
    for i in range(trainer.eval_dataset.__len__()):
        logit = trainer.evaluateasentence(i, info=False)
        logit = logit.detach().cpu().numpy()[0].tolist()
        logits.append(logit)
    with open('logits.txt', 'w') as fp:
        for each in logits:
            fp.writelines(str(each[0]) + ' ' + str(each[1]) + '\n')
